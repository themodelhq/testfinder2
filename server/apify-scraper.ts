...
async function runApifyActor(
  ...
  // maxPaidDatasetItems is REQUIRED for pay-per-result actors — without it
  // the actor runs successfully but writes zero items to the dataset.
  // 40 items at $2.50/1000 = $0.10 per search, well within the $5 free budget.
  const startRes = await fetch(
    `${APIFY_BASE}/acts/${ACTOR_ID}/runs?token=${token}&maxPaidDatasetItems=${PAGE_SIZE}`,
    {
      method: 'POST',
