...
function humanizeScrapeError(
  ...
  switch (code) {
    case 'blocked':
      return `Jumia blocked the request${status}. The hosting provider's IP may be on their blocklist. Try again later, or route the scraper through a proxy.`;
    case 'rate_limited':
      return `Jumia rate-limited the request${status}. Wait a moment and try again.`;
    case 'http_error':
      return `Jumia returned an unexpected HTTP status${status}.`;
    case 'network_error':
      return 'Network error reaching Jumia. Check outbound connectivity from the server.';
    case 'timeout':
      return 'The Jumia request timed out before completing.';
    case 'parse_failed':
