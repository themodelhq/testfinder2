...
3. Set **Build Command** to:
   ...
   ```
   (The `--include=dev` flag is critical. Render sets `NODE_ENV=production`
   for web services, which tells npm to skip devDependencies by default —
   but `vite` and `esbuild` are devDependencies and the build needs them.
   Without the flag you'll see `sh: 1: vite: not found`.)
