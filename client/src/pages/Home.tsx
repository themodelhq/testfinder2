...
export default function Home() {
  ...
  const searchMutation = trpc.jumia.search.useQuery(
    { query: searchQuery, country, page: 1 },
    { enabled: false }
  );

  const skuSearchMutation = trpc.jumia.searchBySkuList.useQuery(
    { skus: skuList.split('\n').filter(s => s.trim()), country },
    { enabled: false }
  );

  const urlSearchMutation = trpc.jumia.searchByUrl.useQuery(
    { url: urlQuery },
    { enabled: false }
  );

  const filterMutation = trpc.jumia.filter.useQuery(
    { products, filters: filters.brands.length > 0 || filters.sellers.length > 0 ? filters : undefined },
    { enabled: false }
  );

  const filteredProducts = useMemo(() => {
    if (filters.brands.length === 0 && filters.sellers.length === 0) {
      return products;
    }
  ...
  const exportMutation = trpc.jumia.exportCsv.useQuery(
  ...
  const handleSkuSearch = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await skuSearchMutation.refetch();
      if (result.data?.products) {
        setProducts(result.data.products);
      }
    } catch (error) {
  ...
  const handleUrlSearch = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await urlSearchMutation.refetch();
      if (result.data?.products) {
        setProducts(result.data.products);
      }
    } catch (error) {
